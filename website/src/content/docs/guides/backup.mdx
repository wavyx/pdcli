...
Where `backup` writes a full snapshot every run, `sync warehouse` appends only what changed
...
Pull-based change capture sees creates and updates only. A record hard-deleted in Pipedrive
simply stops appearing — the NDJSON log keeps its last-known row. Treat the output as an
append log keyed by `(entity, id)` and dedupe / last-write-wins on load, and reconcile
