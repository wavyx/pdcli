...
trades the approximate `Reached` / `Open` columns for an observed `Entered` count and `Won`.

## `metrics coverage`

Are you carrying enough pipeline to hit your number? `metrics coverage` weighs your **open**
pipeline (probability-weighted, the same weighting `pipeline health` uses) against the revenue
still needed to reach a quota, and reports the coverage ratio.
...
The verdict applies the classic 3× rule of thumb against `weighted pipeline ÷ remaining gap`:
