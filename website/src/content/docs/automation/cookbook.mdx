...
- **A single routable type** — `--item-types deal`, `person`, `organization`, or `product` —
  hits that entity's dedicated v2 search endpoint (e.g. `/api/v2/persons/search`). That's a
  narrower OAuth scope and cheaper: **20 rate-limit tokens vs. 40**.
- **Anything else** — no `--item-types`, multiple types, or a non-routable type like `lead`,
  `file`, or `project` — stays on the generic `itemSearch` (40 tokens).
