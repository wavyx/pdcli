...
Every channel below is live. Homebrew installs Node and jq for you; the standalone
tarballs are the only fully self-contained, Node-free option.
...
For macOS and Linux users on Homebrew. `brew` installs Node and jq as dependencies, then
...
For Windows users on Scoop. Installs pdcli via npm (Node is a dependency), and autoupdate
tracks the latest npm release:
