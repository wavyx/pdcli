...
1. **Flags** — `--company`, `--api-token`, `--output`, `--profile`, …
...
domain plus an env token, or an env domain plus a `--api-token` flag, both work.
...
It holds only `activeProfile` and the non-secret per-profile keys above. Credentials are in
the OS keychain, not this file.
