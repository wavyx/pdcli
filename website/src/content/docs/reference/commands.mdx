...
All 154 commands in `pdcli` v0.22.0. Every command also
accepts the [global flags](/pdcli/reference/config/) `--output`, `--jq`, `--fields`, `--resolve-fields`, `--profile`, `--no-color`, `--verbose`, `--no-retry`, `--timeout`, and `--limit`.
Run `pdcli <command> --help` for the live version.
...
| Flag | Description |
| --- | --- |
| `--field` &lt;value&gt; | Field name (built-in or custom) to search on |
| `--value` &lt;value&gt; | Value to match against the field |
...
| Flag | Description |
| --- | --- |
| `--deal` &lt;value&gt; | Deal ID to list mail for |
