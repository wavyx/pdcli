...
const { default: DealFollowerListCommand } =
  await import('../../../../src/commands/deal/follower/list.js')
import { runCmd, mockApi } from '../../../helpers.js'

describe('deal follower list', () => {
  beforeEach(() => {
    nock.cleanAll()
  ...
  it('lists followers across cursor pages as JSON', async () => {
    ...
    const stdout = await runCmd(DealFollowerListCommand, [
      '42',
      '--output',
      'json',
    ])
  ...
  it('requires the deal id positional', async () => {
    await expect(DealFollowerListCommand.run([])).rejects.toThrow()
  })
})
