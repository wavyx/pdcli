...
const { default: DealDeleteCommand } =
  await import('../../../src/commands/deal/delete.js')
...
describe('deal delete', () => {
  ...
  it('deletes after confirmation', async () => {
    ...
    const scope = mockApi()
      .delete('/api/v2/deals/42')
      .reply(200, { success: true, data: { id: 42 } })

    const stdout = await runCmd(DealDeleteCommand, ['42'])
  ...
  it('skips the prompt with --yes', async () => {
    ...
    await runCmd(DealDeleteCommand, ['42', '--yes'])
  ...
  it('aborts without deleting when declined', async () => {
    ...
    try {
      await expect(DealDeleteCommand.run(['42'])).rejects.toThrow(/abort/i)
    } finally {
  ...
  it('emits a JSON object with --output json', async () => {
    ...
    const stdout = await runCmd(DealDeleteCommand, [
      '42',
      '--yes',
      '--output',
      'json',
