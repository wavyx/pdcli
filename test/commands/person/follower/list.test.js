...
const { default: PersonFollowerListCommand } =
  await import('../../../../src/commands/person/follower/list.js')
import { runCmd, mockApi } from '../../../helpers.js'

describe('person follower list', () => {
  beforeEach(() => {
    nock.cleanAll()
  ...
  it('lists followers across cursor pages as JSON', async () => {
    mockApi()
      .get('/api/v2/persons/42/followers')
      .query({ limit: '500' })
      .reply(200, {
        success: true,
        data: [
    ...
    const stdout = await runCmd(PersonFollowerListCommand, [
      '42',
      '--output',
      'json',
    ])
  ...
  it('requires the deal id positional', async () => {
    await expect(PersonFollowerListCommand.run([])).rejects.toThrow()
  })
})
