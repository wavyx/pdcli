...
const { default: NoteDeleteCommand } =
  await import('../../../src/commands/note/delete.js')
...
describe('note delete', () => {
  ...
  it('deletes after confirmation', async () => {
    ...
    const stdout = await runCmd(NoteDeleteCommand, ['5'])
  ...
  it('skips the prompt with --yes', async () => {
    ...
    await runCmd(NoteDeleteCommand, ['5', '--yes'])
  ...
  it('aborts without deleting when declined', async () => {
    ...
    try {
      await expect(NoteDeleteCommand.run(['5'])).rejects.toThrow(/abort/i)
    } finally {
  ...
  it('emits a JSON object with --output json', async () => {
    ...
    const stdout = await runCmd(NoteDeleteCommand, [
      '5',
      '--yes',
      '--output',
      'json',
